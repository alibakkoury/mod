...
def IOUs(rects1, rects2):
        coteb = torch.max(rects1[:, :2].unsqueeze(1), rects2[:, :2].unsqueeze(0))  
        coteh = torch.min(rects1[:, 2:].unsqueeze(1), rects2[:, 2:].unsqueeze(0))  
        intersection_dims = torch.clamp(coteh - coteb, min=0)
        intersection = intersection_dims[:, :, 0] * intersection_dims[:, :, 1]
        #print(intersection)
        aire1 = (rects1[:, 2] - rects1[:, 0]) * (rects1[:, 3] - rects1[:, 1]) 
        #print(rects1[:,0]) 
        aire2 = (rects2[:, 2] - rects2[:, 0]) * (rects2[:, 3] - rects2[:, 1])  
        union = (aire1.unsqueeze(1) + aire2.unsqueeze(0) - intersection)  
...
class BoxRegressionNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1_1 = nn.Conv2d(1024, 256, 1 , padding = 0)  
        self.conv1_2 = nn.Conv2d(256, 512, 3, 2, 1) 

        self.conv2_1 = nn.Conv2d(512, 128, 1, padding = 0)
        self.conv2_2 = nn.Conv2d(128, 256, 3, 2, 1) 

        self.conv3_1 = nn.Conv2d(256, 128, 1, padding = 0)
        self.conv3_2 = nn.Conv2d(128, 256, 3, padding = 0) 

        self.conv4_1 = nn.Conv2d(256, 128, 1, padding = 0)
        self.conv4_2 = nn.Conv2d(128, 256, 3, padding = 0) 
    ...
    def forward(self, x):
        ...
        out = self.activate(self.conv1_1(x)) 
        out = self.activate(self.conv1_2(out)) 
        fmap1 = out 

        out = self.activate(self.conv2_1(out)) 
        out = self.activate(self.conv2_2(out)) 
        fmap2 = out 

        out = self.activate(self.conv3_1(out)) 
        out = self.activate(self.conv3_2(out))  
        fmap3 = out 

        out = self.activate(self.conv4_1(out)) 
        fmap4 = self.activate(self.conv4_2(out))  
...
class ClassificationNet(nn.Module):
        ...
        def __init__(self, nbr_classes):
            ...
            self.loc_conv1 = nn.Conv2d(512 , 6 * 4, 3, 1, 1)
            self.loc_conv2 = nn.Conv2d(1024, 6 * 4, 3, 1, 1)
            self.loc_conv3 = nn.Conv2d(512, 6 * 4, 3, 1, 1)
            self.loc_conv4 = nn.Conv2d(256, 6 * 4, 3, 1, 1)
            self.loc_conv5 = nn.Conv2d(256, 6 * 4, 3, 1, 1)
            self.loc_conv6 = nn.Conv2d(256, 6 * 4, 3, 1, 1)

            self.cla_conv1 = nn.Conv2d(512, 6 * nbr_classes, 3, 1, 1)
...
def default_boxes(ratios = ratios , sizes = sizes , scales = scales):
        default_boxes = []
        for k in range(6):
            for i in range(sizes[k]):
                ...
                for j in range(sizes[k]): 
                    y = (j+0.5) / sizes[k]
                    for r in ratios:
                        default_boxes.append([x, y, scales[k] * sqrt(r), scales[k] / sqrt(r)])
                        if r == 1.:
                            if k<5:
                                default_boxes.append([x, y, sqrt(scales[k] * scales[k + 1]), sqrt(scales[k] * scales[k + 1])])
                            else:
...
def center(rect):
    s= rect.size()[0]
    x , y , w , h = rect[:,0].view((s,1)) , rect[:,1].view((s,1)), rect[:,2].view((s,1)) ,  rect[:,3].view((s,1))
    return torch.cat([x + w / 2, y + h/2, w , h ], 1) 
...
def uncenter(rect):
    ...
    x , y , w , h = rect[:,0].view((s,1)) , rect[:,1].view((s,1)) , rect[:,2].view((s,1)) , rect[:,3].view((s,1))
    #print(x.size())
    return torch.cat([x - w / 2, y - h /2, x + w / 2, y + h /2], dim = 1) 
